        ...
        meta={
            ...
            "slicing": {
                ...
                "pyslm": {
                    ...
                    "project_patterns": sorted(PROJECT_ZIGZAG_PATTERNS),
...
def _validate_pyslm_config(config: SliceConfig) -> None:
    ...
    if config.infill_pattern in PROJECT_ZIGZAG_PATTERNS:
        ...
        if incompatible_overrides:
            raise ValueError(
                "PySLM zigzag/isotropic use the project's bead-aware contour "
                "spacing and do not accept contour geometry overrides: "
                + ", ".join(incompatible_overrides)
            )
...
def _effective_layer_config(
    ...
    if config.material == "R" and config.infill_pattern == "isotropic":
        ...
        return replace(
            ...
            pyslm=(
                _project_pattern_pyslm_config(config.pyslm, hatch_angle)
                if is_cap_layer
                else replace(config.pyslm, hatch_angle=hatch_angle)
            ),
        )
    if config.material == "R" and layer_index == 0:
        return replace(
            ...
            infill_pattern="zigzag",
            infill_density=100.0,
            pyslm=_project_pattern_pyslm_config(
                config.pyslm,
                PART_BOTTOM_ZIGZAG_ANGLE_DEGREES,
            ),
        )
...
def _project_pattern_pyslm_config(settings: Any, hatch_angle: float) -> Any:
    """Keep project-owned cap contours aligned with their infill corridor."""
    ...
    return replace(
        ...
        hatch_distance=None,
        contour_offset=None,
        spot_compensation=None,
        volume_offset_hatch=None,
        num_outer_contours=None,
        num_inner_contours=None,
    )
...
def _pyslm_layer_paths(
    ...
    hatcher.hatchingEnabled = (
        config.infill_pattern not in PROJECT_ZIGZAG_PATTERNS
        and config.infill_pattern != "none"
        and config.infill_density > 0
    )
    ...
    if config.infill_pattern in PROJECT_ZIGZAG_PATTERNS and config.infill_density > 0:
        ...
        infill_roles = ["infill"] * len(infill_paths)
        if config.pyslm.scan_contour_first:
            paths.extend(infill_paths)
            roles.extend(infill_roles)
        else:
            paths = infill_paths + paths
            roles = infill_roles + roles
    return paths, roles
