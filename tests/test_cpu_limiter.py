...
def test_default_cpu_cap_uses_at_most_twelve_available_cores(monkeypatch) -> None:
    monkeypatch.delenv(cpu_limiter.MAX_CPU_CORES_ENV, raising=False)

    assert cpu_limiter.configured_max_cpu_cores(16) == 12
    assert cpu_limiter.configured_max_cpu_cores(3) == 3
...
def test_cpu_cap_environment_override_is_bounded_by_available_cores(monkeypatch) -> None:
    ...
    assert cpu_limiter.configured_max_cpu_cores(8) == 8
