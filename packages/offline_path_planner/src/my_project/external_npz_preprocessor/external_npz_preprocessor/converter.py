...
def source_job_to_parsed_commands(job: SourceJob, params: ProcessParams) -> ParsedCommandList:
    ...
    for layer in job.layers:
        ...
        for material_path in ordered_paths:
            ...
            curve = _validated_spline_or_polyline(
                print_moves,
                source_positions,
                params,
                source_e_profile=source_e_profile,
...
def _validated_spline_or_polyline(
    ...
    *,
    ...
    if not _positions_changed(source_positions, candidate_positions):
        return _make_polyline_curve(
            moves,
            raw="external_npz_polyline",
            e_profile=(
                _absolute_e_profile_from_moves(moves)
                if source_e_profile is not None
                else None
            ),
        )

    candidate_e_profile = None
    if source_e_profile is not None:
        candidate_e_profile = _map_e_profile_to_positions(
            source_positions,
            source_e_profile,
            candidate_positions,
        )
    ...
    smoothed_moves, _ = _print_moves_from_positions(
        ...
        source_e_profile=candidate_e_profile,
    ...
    return _make_polyline_curve(
        ...
        raw=raw,
        e_profile=(
            _absolute_e_profile_from_moves(smoothed_moves)
            if source_e_profile is not None
            else None
        ),
    )
...
def _make_polyline_curve(
    moves: list[MoveCommand],
    raw: str,
    *,
    e_profile: list[float] | None = None,
) -> GlobalCurveCommand:
    first = moves[0]
    last = moves[-1]
    return GlobalCurveCommand(
...
def _absolute_e_profile_from_moves(moves: list[MoveCommand]) -> list[float]:
    if not moves:
        return []
    return [
        float(moves[0].e_val - moves[0].delta_e),
        *[float(move.e_val) for move in moves],
    ]


def _map_e_profile_to_positions(
    source_positions: list[Position],
    source_e_profile: np.ndarray,
    target_positions: list[Position],
) -> np.ndarray:
    """Transport cumulative E through the smoother's path-progress map.

    The smoother can add/remove geometric samples and round corners.  Mapping
    by normalized cumulative path progress preserves every path's total E and
    its local source E density without tying E to a stale XYZ point index.
    """

    source_e = _normalize_source_e_profile(
        source_e_profile,
        len(source_positions),
    )
    assert source_e is not None
    source_s = _cumulative_position_lengths(source_positions)
    target_s = _cumulative_position_lengths(target_positions)
    source_total = float(source_s[-1]) if source_s.size else 0.0
    target_total = float(target_s[-1]) if target_s.size else 0.0
    if source_total <= _EPS or target_total <= _EPS:
        return np.linspace(
            float(source_e[0]),
            float(source_e[-1]),
    ...
    source_progress = source_s / source_total
    target_progress = target_s / target_total
    mapped = np.interp(target_progress, source_progress, source_e)
    mapped[0] = source_e[0]
    mapped[-1] = source_e[-1]
    return mapped.astype(np.float64, copy=False)


def _cumulative_position_lengths(positions: list[Position]) -> np.ndarray:
    cumulative = np.zeros(len(positions), dtype=np.float64)
    for index, (start, end) in enumerate(zip(positions, positions[1:]), start=1):
        cumulative[index] = cumulative[index - 1] + _distance(start, end)
    return cumulative
