...
> 文档性质：最小 UI 实现规划  
> 适用仓库：`kuka_slicer`  
> 当前状态：参数化矩形实体、双正弦曲面、固定蜂窝、对称 smoothstep、主预览适配、JSON 识别及 External Source/Core 路径桥接已实现；旧曲面映射器保持独立。
...
2. 新流程不输入蜂窝 STL：外边界固定为 UI 定义的矩形，STL 仅保留为旧方案对比输入，不参与新格栅拓扑；
...
| UI 名称 | 配置字段 | 含义 |
| --- | --- | --- |
| 设计墙宽 | `wall_width_mm` | 用于格栅几何和填充率估计的名义结构墙宽，只允许 2 mm 的正整数倍 |
| 目标六边形边长 | `base_cell_size_mm` | 沿承载曲面测量的名义六边形边长，决定格栅疏密 |

喷嘴基准沉积线宽固定为 2 mm。`wall_width_mm=2n` 时，路径桥接输出 `n` 条相隔 2 mm 的沉积道；实际挤出截面积和 E 换算仍由主界面选中的 Core 树脂工艺参数明确决定。
...
- 多种层间渐变函数；
- 手工切缝、手工 LSCM 锚点；
- 外部 STL 驱动的蜂窝拓扑恢复；新格栅使用自身稳定边图的最小不重复 trail cover、零 E 图连接和分区 travel；
